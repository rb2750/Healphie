...
class ChartState extends State<Charts> {
  ...
  Widget makeButton(String text, Border border, TimeScale newTimeScale,
      ChartType newChartType, bool selected) {
    return InkWell(
      onTap: () {
      ...
      child: new Container(
        ...
        decoration: new BoxDecoration(
            border: border, color: selected ? Colors.black12 : Colors.white),
        child: new Center(
            child: Text(
          text,
          style: TextStyle(
  ...
  Widget build(BuildContext context) {
    ...
    if (chartType == ChartType.BAR) {
      chartWidget = FutureBuilder<
              List<charts.Series<OrdinalHappinessBar, String>>>(
          initialData: new List<charts.Series<OrdinalHappinessBar, String>>(),
          future: _barData,
          builder: (BuildContext context,
              AsyncSnapshot<List<charts.Series<OrdinalHappinessBar, String>>>
                  snapshot) {
            Widget chart;
      ...
      chartWidget = FutureBuilder<
              List<charts.Series<OrdinalHappinessPie, String>>>(
          initialData: new List<charts.Series<OrdinalHappinessPie, String>>(),
          future: _pieData,
          builder: (BuildContext context,
              AsyncSnapshot<List<charts.Series<OrdinalHappinessPie, String>>>
                  snapshot) {
                ...
                defaultRenderer:
                  ...
                  new charts.ArcLabelDecorator(
                      ...
                      insideLabelStyleSpec:
                          new charts.TextStyleSpec(color: charts.MaterialPalette.black,fontSize: 18))
                ]));
          });
    }
      ...
      return Container(
        ...
        Container(
            ...
            child: Row(children: <Widget>[
              makeButton("Day", Border(right: border), TimeScale.DAY, chartType,
                  chartTimeScale == TimeScale.DAY),
              makeButton("Month", Border(right: border), TimeScale.MONTH,
                  chartType, chartTimeScale == TimeScale.MONTH),
              makeButton("Year", Border(right: border), TimeScale.YEAR,
                  chartType, chartTimeScale == TimeScale.YEAR),
              Spacer(),
              makeButton("Pie", Border(left: border), chartTimeScale,
                  ChartType.PIE, chartType == ChartType.PIE),
              makeButton("Bar", Border(left: border), chartTimeScale,
                  ChartType.BAR, chartType == ChartType.BAR)
            ])),
        Container(
            height: viewportConstraints.maxHeight - 51,
            child: chartWidget,
  ...
  Future<List<charts.Series<OrdinalHappinessBar, String>>>
      createBarData() async {
    ...
    for (var key in prefs.getKeys()) {
      ...
      if (chartTimeScale == TimeScale.DAY) {
        relevant = !DateTime.now()
            .isBefore(datetime.subtract(new Duration(hours: 12)));
      } else if (chartTimeScale == TimeScale.MONTH) {
        relevant =
            !DateTime.now().isBefore(datetime.subtract(new Duration(days: 30)));
      } else if (chartTimeScale == TimeScale.YEAR) {
        relevant = !DateTime.now()
            .isBefore(datetime.subtract(new Duration(days: 365)));
      }
    ...
    return [
      new charts.Series<OrdinalHappinessBar, String>(
        id: 'Happiness',
        domainFn: (OrdinalHappinessBar sales, _) =>
            new DateFormat("HH:mm").format(sales.date),
  ...
  Future<List<charts.Series<OrdinalHappinessPie, String>>>
      createPieData() async {
    ...
    for (var key in prefs.getKeys()) {
      ...
      if (chartTimeScale == TimeScale.DAY) {
        relevant =
            !DateTime.now().isBefore(datetime.subtract(new Duration(days: 1)));
      } else if (chartTimeScale == TimeScale.MONTH) {
        relevant =
            !DateTime.now().isBefore(datetime.subtract(new Duration(days: 30)));
      } else if (chartTimeScale == TimeScale.YEAR) {
      ...
      if (relevant) {
        dataCount.update(prefs.getInt(key), (value) => ++value,
            ifAbsent: () => 1);
      }
    }
